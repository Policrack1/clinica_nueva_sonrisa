...
import { format, startOfMonth, endOfMonth, eachDayOfInterval,
         ...
         subMonths, addWeeks, subWeeks, addDays, subDays, getDay } from 'date-fns'
...
export default function AdminAgenda() {
  ...
  const [current,  setCurrent]  = useState(new Date(2026, 3, 18)) // Abril 2026
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-slate-100 flex-wrap">
          <button onClick={() => nav(-1)} className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center hover:border-blue-400 hover:text-blue-600 transition-colors">
            <ChevronLeft size={15} />
          </button>
          <span className="font-sora font-bold text-slate-800 capitalize text-[15px]">{headerLabel()}</span>
          <button onClick={() => nav(1)} className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center hover:border-blue-400 hover:text-blue-600 transition-colors">
